...
class GaussianMLPValueFunction(ValueFunction, GaussianMLPModule):
    ...
    def __init__(self,
        ...
        ValueFunction.__init__(self, env_spec, name)
    ...
    def predict(self, obs):
        ...
        """
        normalized_obs = (torch.Tensor(obs) -
                          self._obs_dist['mean']) / self._obs_dist['std']
        with torch.no_grad():
            dist = self.forward(normalized_obs)
        mean = dist.mean * self._return_dist['std'] + self._return_dist['mean']
        return mean.flatten()
